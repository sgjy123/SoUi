...
2. **如果用户选择参考框架**：研究该框架的 API 设计，但保持 SoUi 的设计风格
3. **参考现有实现**：阅读 Button、Icon、Typography 等组件
4. **遵循命名规范**：类名 `soui-` 前缀，PascalCase 组件名
5. **完整导出类型**：Props、枚举、配置接口都要导出
6. **文档要详细**：API、示例、设计原则缺一不可
7. **如果参考了框架**：必须在文档中添加“参考来源”章节
8. **示例要实用**：覆盖常见使用场景
9. **构建要验证**：确保无编译错误
